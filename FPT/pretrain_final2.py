...
class PretrainDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if length != 0:
            ...
            # 选择回复
            # 正回复
            pos_response = self.all_docs[sample["doc_id"]][sample["line"] + 3]
            # 负回复
            rand = random.random()
            if self.task == "ubuntu":
                rand = 0
            # 从同一个对话中选择一个作为负样本
            # if rand > 0.85:
            #     print("sample from same context")
            #     samedoc = self.all_docs[sample["doc_id"]]
            #     linenum = random.randrange(len(samedoc))
            #     while linenum == sample["line"] + 3 or linenum == sample["line"] + 2:
            #         linenum = random.randrange(len(samedoc))
            #     neg_response = samedoc[linenum]
            # # 从其他对话中选择一个作为负样本
            # else:
            #     # print("sample from other context")
            neg_response = self.get_random_line(sample)
...
class NeuralNetwork(nn.Module):
    ...
    def __init__(self, args):
        ...
        # self.bert_model = model_class.from_pretrained("./PT_checkpoint/e_commerce_9_epoch")
