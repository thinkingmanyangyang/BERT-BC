...
def PT_data():
    ...
    data = open('e_commerce_data/train.txt', 'r', encoding='utf8').readlines()
    data = [sent.split('\n')[0].split('\t') for sent in data]
    y = [int(a[0]) for a in data]
    cr = [[sen for sen in a[1:]] for a in data]
    crnew=[]
    ...
    crnew=crnew
    pickle.dump(crnew, file=open("e_commerce_data/e_commerce_post_train.pkl", 'wb'))
    with open('e_commerce_data/e_commerce_post_train.json', 'w', encoding='utf8') as f:
        json.dump(crnew, f, ensure_ascii=False, indent=4)
